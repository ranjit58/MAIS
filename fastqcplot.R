...
raw_data <- read.table(file="qcplot.txt",header=T,sep="\t")  #read file
cc<-ncol(raw_data)  #count column
cr<-nrow(raw_data)  #count row
cr5 = cr+20          #add 5 to cr, for creating space for label

usable<- raw_data[,2:cc]     #store usable data for plot
cc<-ncol(usable)  #count column

plot(c(1:cr), type="l", main = "FASTQC plot for all samples", ylab = "Q Score", xlab= "base position", ylim= c(1,40),xlim=c(1,cr5),col="white",xaxs = "i",yaxs="i")
gcc<- 39
gcr<- cr5-1
grid(gcr,gcc)

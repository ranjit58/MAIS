...
#-------------------------------------------------------------------------
# Name - merge_otu_seq_norm.py otu_table.txt seqs.fna_rep_set.fasta normalized_otu.txt 0.01
# Desc - The program merge otu table and 16S sequence and convert the abundance information into proportion (i.e. normalize)
# the OTUs are filtered such that any OTU which has total abundance less than 0.1% are filtered out.
...
for line in input_otu:
  if re.match('#',line):
    ...
    for i in range(1,len(linesplit)-1):
      ...
      output.write(str(round(otu_ratio,3)) + "\t")
...
print "Reading normalized file and creating a filtered version filtered.",sys.argv[3],"\n"

input=open(sys.argv[3],"r")
output=open("filtered."+ sys.argv[3],"w")
...
for line in input:
  if re.match('#',line):
    ...
    for i in range(1,len(linesplit)-2):
      otu_sum = otu_sum + float(linesplit[i])
    if otu_sum >= float(sys.argv[4]):
      output.write(line + "\n")
